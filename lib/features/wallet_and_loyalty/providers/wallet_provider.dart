...
import 'package:mazar/features/wallet_and_loyalty/domain/models/wallet_bonus_model.dart';
import 'package:mazar/features/wallet_and_loyalty/domain/models/wallet_filter_model.dart';
import 'package:mazar/features/wallet_and_loyalty/domain/models/wallet_model.dart';
...
class WalletAndLoyaltyProvider with ChangeNotifier {
  ...
  Future<bool> pointToWallet(int point, bool fromWallet) async {
    ...
    notifyListeners();
    ApiResponseModel apiResponse =
        await walletRepo!.pointToWallet(point: point);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      isSuccess = true;
  ...
  void insertFilterList() {
    ...
    for (int i = 0; i < AppConstants.walletTransactionSortingList.length; i++) {
      _walletFilterList.add(WalletFilterModel.fromJson(
